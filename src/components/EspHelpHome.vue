...
<style scoped>
	...
	.main>div.content_box>div>span{width: 15%;display: inline-block;text-align: right;margin:26px 12px 0 0;}
	.main>div.content_box>div p{display: inline-block;width: 80%;text-align: left;}
	.main>div.content_box>div>.el-input,.main>div .el-textarea,.main>div .el-select{width: 80%;}
	.main>div.content_box>div .el-textarea{margin-top:26px;}
	.foot{margin:20px 0;}
	.ImageBox{margin-top:20px;}
	@media screen and (max-width: 800px) {
		.main>div.content_box>div>span{width: 22%}
		.main>div.content_box>div>.el-input,.main>div .el-textarea,.main>div .el-select{width: 73%;}
	}
	...
	.header{box-sizing:border-box;overflow:hidden;padding:20px 0;border-bottom:1px solid #ccc;}
	.nav{position: fixed;}
	.content{margin:0 auto;overflow:hidden;}
	.left{float: left;}
	.right{float: right}
	.tip a{display: inline-block;padding: 0 20px 10px 20px;color: #000}
	.tip a:hover{text-decoration:none;}
	.tip a.active{background: url(../assets/active.png) no-repeat center bottom;color:#618DCF;}
</style>
<script>
...
export default {
  ...
  methods:{
	...
	handleExceed(files, fileList){
		this.$layer.msg(`当前限制选择 3 个文件，本次选择了 ${files.length} 个文件，共选择了 ${files.length + fileList.length} 个文件`);
	},
	submitFile(file){
		let _that = this;
		const isLt2M = file.size / 1024 / 1024 < 2;
		if(!isLt2M){
			this.$layer.msg('文件不得大于2M');
	...
	submit(){
		...
		let desc = this.desc;
		if(!SCode){
			this.$layer.msg('请填写学校');
			return;
		}else if(!PCode){
			this.$layer.msg('请填写所属项目');
			return;
		}else if(!uname){
			this.$layer.msg('请填写姓名');
			return;
		}else if(!un){
			this.$layer.msg('请填写登陆账号');
			return;
		}else if(!uphone){
			this.$layer.msg('请填写联系方式');
			return;
		}else if(!desc){
			this.$layer.msg('请填写问题描述');
			return;
		}
		this.SendWX();
	}
